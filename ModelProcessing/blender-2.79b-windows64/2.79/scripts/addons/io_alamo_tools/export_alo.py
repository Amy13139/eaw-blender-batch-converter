...
class ALO_Exporter(bpy.types.Operator):
    ...
    filepath = StringProperty(name="File Path", description="Filepath used for exporting the ALO input_file", maxlen=1024,
                              default="")
    ...
    def execute(self, context):  # execute() is called by blender when running the operator.
        ...
        def create_skeleton():
            ...
            file.write(header + data)  # write data to input_file
        ...
        def create_mesh_info_chunk(object, material_list):
            ...
            mesh_information_chunk += utils.pack_float(min(x_list))  # write min/max of input_file into input_file
        ...
        def create_mesh(mesh_list, bone_name_per_alo_index):
            for object in mesh_list:
                ...
                modifier = object.modifiers.new('Triangulate', 'TRIANGULATE')
                ...
                mesh = object.to_mesh(scene=bpy.context.scene, settings='PREVIEW', apply_modifiers=True)
        ...
        def create_animation_mapping(bm, object, material, bone_name_per_alo_index):
                        ...
                        for face in bm.faces:
                            if (material.name == object.material_slots[face.material_index].name):
                                for vertex in face.verts:
                                    dvert = vertex[dvert_lay]
                                    if dvert.items()[0][0] in group_index_list:
                                        pass
                                    else:
                                        group_index_list.append(dvert.items()[0][0])
        ...
        def submesh_vertex_face_data(bm, object, material, uses_bump, mesh):
            ...
            for face in faces:
                for loop in face.loops: #loops are vertices per UV
                    ...
                    if store_vertex:
                        ...
                        if dvert_lay != None:
                            ...
                            index = dvert.items()[0][0]
                            vertex.bone_index = index
                ...
                if uses_bump:
                    tangent_bitangent.append(face.calc_tangent_edge())

            if uses_bump:
                index = 0
                while index + 2 < len(face_indices):
                    calculateTangentBitangent(vertices[face_indices[index]], vertices[face_indices[index + 1]], vertices[face_indices[index + 2]])
                    index += 3
            return [vertices, face_indices]

        def shadow_vertex_face_data(bm):
        ...
        def create_sub_mesh_data_chunk(mesh, material, object, bone_name_per_alo_index):
            ...
            if is_shadow:
                vertex_face_data = shadow_vertex_face_data(bm)
            else:
            ...
            mapping_data = create_animation_mapping(bm, object, material, bone_name_per_alo_index)
            ...
            else:   #if object has no armature modifier bone index is always 0
                for vertex in vertices:
                    vertex.bone_index = 0
        ...
        def calculateTangentBitangent(v0, v1, v2):
                ...
                bitangent = mathutils.Vector((0, 0, 0))
        ...
        def mat_tex_chunk(type, name):
            ...
            chunk += bytes(type, 'utf-8') + b"\x00"  # add input_type
            chunk += b"\x02"  # mini chunk header
            chunk += utils.pack_u_char(len(name) + 1)  # size
            chunk += bytes(name, 'utf-8') + b"\x00"  # add input_type
            return chunk
        ...
        def hide(hiddenList):
            counter = 0
            for object in bpy.data.objects:
                object.hide =  hiddenList[counter]
                counter += 1
        ...
        def export_list():
            ...
            for object in bpy.data.objects:
                if object.type == 'MESH':  # check if object is mesh
                    exported = False
                    for counter, layer in enumerate(object.layers):
                        if layer == True:
                            if scene.layers[counter] == True:
                                if exported == False:
                                    mesh_list.append(object)
            return mesh_list

        def unhide_armature_layer():
            scene = bpy.context.scene
            armature = utils.find_armature()
            if armature == None:
                return -1
            for counter, layer in enumerate(armature.layers):
                if layer == True:
                    if scene.layers[counter] == False:
                        scene.layers[counter] = True
                        return counter
                    else:
                        return -1

        def hide_armature_layer(armature_layer):
            if armature_layer == -1:
                return
            else:
                scene = bpy.context.scene
                scene.layers[armature_layer] = False
        ...
        hiddenList = unhide()
        armature_layer = unhide_armature_layer()

        path = self.properties.filepath

        global file
        file = open(path, 'wb')  # open input_file in read binary mode

        bone_name_per_alo_index = create_skeleton()
        create_mesh(mesh_list, bone_name_per_alo_index)
        create_connections(mesh_list)

        hide_armature_layer(armature_layer)
